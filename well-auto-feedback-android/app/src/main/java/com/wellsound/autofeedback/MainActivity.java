...
public class MainActivity extends Activity {
    ...
    Spinner target,operation,fxSlot,side;
    Button startStop;
    ...
    final float[] flatAverage=new float[31];
    int flatFrames=0;
    ...
    void buildUi(){
        ...
        root.addView(label("MODE"));operation=spin(new String[]{"FEEDBACK","FLAT EQ"},0);root.addView(operation,lp());

        root.addView(label("DUAL GEQ EFFECT"));
        LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);
        fxSlot=spin(new String[]{"FX5","FX6","FX7","FX8"},1);side=spin(new String[]{"A","B"},0);
        row.addView(fxSlot,new LinearLayout.LayoutParams(0,-2,1));row.addView(side,new LinearLayout.LayoutParams(0,-2,1));root.addView(row,lp());
        ...
        Button undoBtn=btn("UNDO LAST GEQ CHANGE");root.addView(undoBtn,lp());
        Button restore=btn("RESTORE ORIGINAL GEQ");root.addView(restore,lp());
        log=tv("READY",13,false);log.setTextColor(Color.LTGRAY);root.addView(log,lp());
        setContentView(sc);

        connect.setOnClickListener(v->connectMixer());
        startStop.setOnClickListener(v->{if(engineRunning)stopEngine();else startEngine();});
        ...
        operation.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){
            public void onItemSelected(android.widget.AdapterView<?> p,View v,int pos,long id){candidate.setVisibility(pos==0?View.VISIBLE:View.GONE);if(engineRunning)stopEngine();resetFlatAverage();}
        ...
        android.widget.AdapterView.OnItemSelectedListener change=new android.widget.AdapterView.OnItemSelectedListener(){
            public void onItemSelected(android.widget.AdapterView<?> p,View v,int pos,long id){if(engineRunning)stopEngine();if(connected){requestGeq();configureRta();}}
            public void onNothingSelected(android.widget.AdapterView<?> p){}
        };
        target.setOnItemSelectedListener(change);fxSlot.setOnItemSelectedListener(change);side.setOnItemSelectedListener(change);
    }

    String[] makeTargets(){String[] a=new String[17];a[0]="Main LR";for(int i=1;i<=16;i++)a[i]=String.format(Locale.US,"Bus %02d",i);return a;}
    TextView tv(String s,int sz,boolean bold){TextView v=new TextView(this);v.setText(s);v.setTextColor(Color.WHITE);v.setTextSize(sz);v.setPadding(0,8,0,8);if(bold)v.setTypeface(null,1);return v;}
    ...
    int fx(){return 5+fxSlot.getSelectedItemPosition();}
    int param(int band){return 1+band+(side.getSelectedItemPosition()==0?0:32);}
    String pa(int p){return String.format(Locale.US,"/fx/%d/par/%02d",fx(),p);}
    ...
    void connectMixer(){
        mixerIp=ip.getText().toString().trim();if(mixerIp.length()<7){toast("ใส่ IP M32R ก่อน");return;}
        stopConnection();status.setText("CONNECTING...");status.setTextColor(Color.rgb(251,191,36));startStop.setEnabled(false);
        new Thread(()->{try{
            socket=new DatagramSocket();socket.setSoTimeout(700);running=true;sendNow("/info");
            byte[] b=new byte[4096];DatagramPacket p=new DatagramPacket(b,b.length);socket.receive(p);
            connected=true;lastRx=System.currentTimeMillis();
            runOnUiThread(()->{status.setText("M32R CONNECTED");status.setTextColor(Color.rgb(52,211,153));startStop.setEnabled(true);});
            requestGeq();configureRtaNow();new Thread(this::rxLoop,"m32-rx").start();new Thread(this::keepAlive,"m32-keep").start();ui("Connected "+mixerIp+":10023");
        }catch(Exception e){runOnUiThread(()->{status.setText("NO RESPONSE");status.setTextColor(Color.rgb(248,113,113));});ui(e.toString());stopConnection();}},"m32-connect").start();
    }

    void startEngine(){
        if(!connected){toast("ต้อง CONNECT M32R ก่อน");return;}
        if(!geqOk()){toast("FX ที่เลือกยังไม่ยืนยันว่าเป็น Dual GEQ");requestGeq();return;}
        if(snapshot.size()<20){toast("กำลังอ่านค่า GEQ เดิม กรุณากด START อีกครั้งในอีกสักครู่");requestGeq();return;}
        engineRunning=true;candStart=0;candBand=-1;resetFlatAverage();lastFlatApply=0;
        runOnUiThread(()->{startStop.setText("■ STOP REALTIME");runState.setText("ENGINE: RUNNING • "+operation.getSelectedItem());runState.setTextColor(Color.rgb(52,211,153));});
        ui("START REALTIME • "+operation.getSelectedItem()+" • FX"+fx()+"/"+(side.getSelectedItemPosition()==0?"A":"B"));
    }
    void stopEngine(){engineRunning=false;candStart=0;candBand=-1;resetFlatAverage();runOnUiThread(()->{startStop.setText("▶ START REALTIME");runState.setText("ENGINE: STOPPED");runState.setTextColor(Color.rgb(148,163,184));candidate.setText("Feedback Candidate: —");});ui("STOP REALTIME");}
    void resetFlatAverage(){Arrays.fill(flatAverage,0f);flatFrames=0;}

    void stopConnection(){stopEngine();running=false;connected=false;if(socket!=null){try{socket.close();}catch(Exception ignored){}socket=null;}}
    ...
    void send(String addr,Object...args){new Thread(()->{try{sendNow(addr,args);}catch(Exception ignored){}},"m32-tx").start();}
    void configureRta(){send("/-prefs/rta/source",source());send("/-prefs/rta/pos",0);send("/-prefs/rta/det",1);}
    void configureRtaNow()throws Exception{sendNow("/-prefs/rta/source",source());sendNow("/-prefs/rta/pos",0);sendNow("/-prefs/rta/det",1);}
    ...
    void keepAlive(){long sub=0;while(running){try{long n=System.currentTimeMillis();sendNow("/xremote");if(n-sub>2500||lastRta==0||n-lastRta>1800){configureRtaNow();sendNow("/meters","/meters/15",1);sub=n;if(lastRta>0&&n-lastRta>1800)runOnUiThread(()->rtaState.setText("RTA: AUTO-RECONNECT…"));}Thread.sleep(700);}catch(Exception ignored){}}}
    void rxLoop(){byte[] buf=new byte[65535];while(running){try{DatagramPacket p=new DatagramPacket(buf,buf.length);socket.receive(p);lastRx=System.currentTimeMillis();parse(Arrays.copyOf(p.getData(),p.getLength()));}catch(SocketTimeoutException e){if(System.currentTimeMillis()-lastRx>4500){engineRunning=false;runOnUiThread(()->{status.setText("M32R LINK LOST");status.setTextColor(Color.rgb(248,113,113));startStop.setText("▶ START REALTIME");runState.setText("ENGINE: STOPPED • LINK LOST");});}}catch(Exception ignored){}}}

    void parse(byte[] d){try{
        ...
        if(a.equals("/meters/15")&&tags.contains("b")){
            ...
            if(n>=200&&o[0]+n<=d.length){decode(Arrays.copyOfRange(d,o[0],o[0]+n));lastRta=System.currentTimeMillis();runOnUiThread(()->rtaState.setText("RTA: LIVE • "+targetName()));}
            return;
        }
        if(a.equals("/fx/"+fx()+"/type")&&tags.length()>1&&tags.charAt(1)=='i'){
            fxType=ByteBuffer.wrap(d,o[0],4).order(ByteOrder.BIG_ENDIAN).getInt();
            runOnUiThread(()->geqState.setText("GEQ: FX"+fx()+" / "+(side.getSelectedItemPosition()==0?"A":"B")+" • "+(geqOk()?"Dual GEQ READY":"NOT DUAL GEQ • type "+fxType)));
            return;
        }
        String pre="/fx/"+fx()+"/par/";
        if(a.startsWith(pre)&&tags.length()>1&&tags.charAt(1)=='f'){
            int p=Integer.parseInt(a.substring(pre.length()));float v=ByteBuffer.wrap(d,o[0],4).order(ByteOrder.BIG_ENDIAN).getFloat();current.put(p,v);if(!snapshot.containsKey(p))snapshot.put(p,v);
        }
    }catch(Exception ignored){}}

    void decode(byte[] b){try{
        int off=b.length>=204?4:0;if(b.length-off<200)return;ByteBuffer bb=ByteBuffer.wrap(b,off,200).order(ByteOrder.LITTLE_ENDIAN);for(int i=0;i<100;i++)rta[i]=bb.getShort()/256f;
        runOnUiThread(()->spectrum.setData(rta));
        if(engineRunning){if(operation.getSelectedItemPosition()==0)detectFeedback();else processFlatRealtime();}
    }catch(Exception ignored){}}
    ...
    void detectFeedback(){
        ...
        for(int i=18;i<94;i++){
            ...
            float s=0;int n=0;for(int j=Math.max(0,i-4);j<Math.min(100,i+5);j++)if(Math.abs(j-i)>=2){s+=rta[j];n++;}
            float ex=rta[i]-s/n;if(ex>best&&ex>6.5f){best=ex;bi=i;}
        }
        if(bi<0){candStart=0;candBand=-1;runOnUiThread(()->candidate.setText("Feedback Candidate: —"));return;}
        int band=nearestBand(RTAF[bi]);int f=(int)GEQF[band];
    ...
    void pushUndo(){undo.push(new HashMap<Integer,Float>(current));if(undo.size()>12)undo.removeLast();}
    void writeBand(int i,float db){float v=norm(db);int p=param(i);current.put(p,v);send(pa(p),v);}
    float norm(float db){return Math.max(0,Math.min(1,(db+15f)/30f));}
    float denorm(float v){return v*30f-15f;}
    void undoLast(){if(engineRunning)stopEngine();if(undo.isEmpty()){toast("ยังไม่มีค่าให้ Undo");return;}HashMap<Integer,Float> m=undo.pop();for(Map.Entry<Integer,Float> e:m.entrySet())send(pa(e.getKey()),e.getValue());current.clear();current.putAll(m);ui("UNDO LAST GEQ CHANGE");}
    void restore(){if(engineRunning)stopEngine();if(snapshot.isEmpty()){toast("ยังไม่มี Original GEQ snapshot");return;}for(Map.Entry<Integer,Float> e:snapshot.entrySet())send(pa(e.getKey()),e.getValue());current.clear();current.putAll(snapshot);ui("ORIGINAL GEQ RESTORED");}
    ...
    byte[] osc(String addr,Object...args)throws Exception{
        ...
        out.write(pad(addr));out.write(pad(tags.toString()));out.write(pay.toByteArray());return out.toByteArray();
    }
    ...
    @Override protected void onDestroy(){stopConnection();super.onDestroy();}

    public static class SpectrumView extends View{
        Paint p=new Paint(Paint.ANTI_ALIAS_FLAG);float[] data=new float[100];
        public SpectrumView(android.content.Context c){super(c);Arrays.fill(data,-90f);}
        void setData(float[] d){data=d.clone();invalidate();}
        @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight();p.setColor(Color.rgb(32,45,60));p.setStrokeWidth(1);for(int i=1;i<10;i++){float y=h*i/10f;c.drawLine(0,y,w,y,p);}p.setColor(Color.rgb(56,189,248));p.setStrokeWidth(3);Path path=new Path();for(int i=0;i<data.length;i++){float x=w*i/(data.length-1f);float db=Math.max(-90,Math.min(0,data[i]));float y=(-db/90f)*(h-12)+6;if(i==0)path.moveTo(x,y);else path.lineTo(x,y);}c.drawPath(path,p);}
    }
}
